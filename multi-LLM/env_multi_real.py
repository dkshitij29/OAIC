...
class Environ:
    def __init__(self, flow_number_highest, flow_number_lowest, path, time_stamp):
        ...
        self.total_data_rate_states_slice_1 = np.array([31,31,31,31,31,5,5,5,5,5])
        self.total_data_rate_states_slice_2 = np.array([25,25,25,25,25,10,10,10,10,10])
        ...
        # self.shareacity = 1600 #Mbps 
        self.compute_capacity = 25 #Mbps
        self.shareacity = 100 
    ...
    def step(self, Action1, Action2, ts,option):
        ...
        # self.sharealloc = np.round(np.clip(self.shareacity/(np.sum(self.sharealloc)+0.0001), 0.0001, 1)*self.sharealloc)
        # self.data_rate = self.sharealloc * np.log2(1 + SNR_array)
        data_rate = get_data_rate()
        self.data_rate = np.array(data_rate)

        self.request_data_rate = np.array([DARA_RATE_REQUEST[0], DARA_RATE_REQUEST[1]])
        ...
        if option=='test' or option=='train':
            ...
            for phi in range(len(self.flow_list)):
                ...
                flow.data_rate =  DARA_RATE_REQUEST[phi]
    ...
    def Reset(self,option=None,eps=None):
        ...
        # if service_type==0: # 0 is the eMBB
            ...
            data_rate_request = DARA_RATE_REQUEST[phi]
